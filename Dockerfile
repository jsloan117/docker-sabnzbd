...
ENV VERSION=2.3.8 PAR2=0.8.0

RUN apk add --no-cache ca-certificates py-six py-cryptography py-enum34 \
                       py-cffi py-cheetah py-openssl openssl unzip unrar p7zip python2 \
                       py-pip build-base libgomp automake autoconf python2-dev \
                       bash tini shadow supervisor \
    && wget -O- https://github.com/sabnzbd/sabnzbd/releases/download/$VERSION/SABnzbd-$VERSION-src.tar.gz | tar -zx \
    && mv SABnzbd-*/ /sabnzbd \
